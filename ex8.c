...
int buscar (int* v, int l, int m, int r, int elem) {

	if ((m > l) && (m < r)) {
		if (elem == v[m]) return m;
		else if (elem < v[m]) return buscar(v, l, (((m - l) / 2) + l), m, elem);
		else return buscar(v, m, (((r - m) / 2) + m), r, elem);
	} else return -999;
}
...
int main(int argc, char* argv[]) {
	...
	for (i = 0; i < buscas; i++) {
		j = rand() % limiter;
		r = buscar(vetor, 0, (MAX / 2), (MAX - 1), j);
